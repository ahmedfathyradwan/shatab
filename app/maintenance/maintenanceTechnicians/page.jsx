...
import freelancerTypes from '../../data/smallMaintenanceTechniciansTypes'; 
import styles from './maintenanceTechnicians.module.css';
...
export default function MaintenanceTechniciansPage() {
  ...
  const filteredFreelancers = selectedType
    ...
    : freelancers.filter((f) => f.specialty.toLowerCase().includes("صيانة"));

  return (
    <div className={styles.wrap}>
      <header className={styles.header}>
        <h1>فنيي الصيانة المتاحين</h1>
        <p className={styles.lead}>قائمة مختصرة بالفنيين والخبراء المتخصصين في الصيانة</p>
        ...
        <div className={styles.filterBox}>
          ...
          >
            ...
            {freelancerTypes
              .filter((type) => type.toLowerCase().includes("صيانة"))
              .map((type, index) => (
      ...
      <main className={styles.grid}>
        {filteredFreelancers.length > 0 ? (
            ...
            <article key={c.id} className={styles.card}>
              ...
              <div className={styles.body}>
                ...
                <div className={styles.actions}>
                  <Link href={`/maintenanceTechnicians/${c.id}`} className={styles.button}>
                    تفاصيل أكتر
                  </Link>
                </div>
              </div>
          ...
          <p className={styles.noResults}>لا يوجد فنيين صيانة بهذا التخصص حالياً</p>
        )}
      </main>
