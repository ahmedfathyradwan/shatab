...
import designSpecialties from '../../../data/designSpecialties.json';
...
export default function DesignerRegisterPage() {
  ...
  const specialtyOptions = designSpecialties.map((item) => ({ label: item, value: item }));
  ...
  const handleSubmit = (e) => {
    ...
    if (!specialty || !governorate) {
      setError('يرجى اختيار التخصص والمحافظة');
      return;
    }
  ...
  return (
    <div className={styles.container}>
      <div className={styles.formContainer}>
        <h2 className={styles.title}>إنشاء حساب مصمم أو رسام</h2>
        ...
        <form className={styles.form} onSubmit={handleSubmit}>
          ...
          {isClient && (
            <>
              <div className={styles.formGroup}>
                <label className={styles.label}>نوع التخصص</label>
                <MySelect
                  key="designer-specialty-select"
                  value={specialty}
